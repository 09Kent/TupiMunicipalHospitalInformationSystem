...
<main class="flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8">
  ...
  <div class="no-print flex items-center justify-between mb-4">
    <a href="<?= route('register.dashboard') ?>" class="inline-flex items-center gap-2 text-xs font-bold text-slate-500 hover:text-blue-600 transition bg-white/90 px-3.5 py-2 rounded-xl border border-slate-200/90 shadow-2xs">
...
<script>
  window.SERVER_DATA = {
    ...
    apiEndpoints: {
      classify: '<?= base_url('/api/register/classify') ?>',
      submit: '<?= base_url('/api/register/submit') ?>',
      doctors: '<?= base_url('api/doctors/index.php') ?>',
      symptoms: '<?= base_url('/api/register/symptoms') ?>'
    },
    baseUrl: '<?= base_url() ?>'
  };
</script>
...
<!-- Scripts -->
<script src="<?= base_url(asset('section/register/js/data.js')) ?>"></script>
<script src="<?= base_url(asset('section/register/js/body-map.js')) ?>"></script>
<script src="<?= base_url(asset('section/register/assets/js/registration.js')) ?>"></script>
@endsection
